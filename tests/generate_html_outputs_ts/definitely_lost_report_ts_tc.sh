...
function generate_memcheck_report()
{
    ...
    # Call the memcheck runner with it's output set to ${test_out_dir}definitely_lost.memcheck
    $memcheck_runner -o${test_out_dir}definitely_lost -- ${definitely_lost_bin} > "${test_std_output}" 2> "${test_err_output}"

    expect_file "${test_out_dir}definitely_lost.memcheck"

    anonymize_memcheck_file "${test_out_dir}definitely_lost.memcheck"
}
...
function test_definitely_lost_report()
{
    ...
    local test_ref_report_dir="${current_full_path}/ref/definitely_lost_report/"
    ...
    expect_file "${report_out_dir}definitely_lost.memcheck.html.part"
