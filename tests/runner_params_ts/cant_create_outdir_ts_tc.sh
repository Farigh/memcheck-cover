...
function test_cant_create_outdir()
{
    ...
    # Call the memcheck runner (Use true as it's a simple, 0 returning cmd)
    "${memcheck_runner}" -o "${dummy_runner_ouput_dir}/dummy" -- true > "${test_std_output}" 2> "${test_err_output}"
